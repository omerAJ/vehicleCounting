...
import db_utils
import datetime
from utils import smooth_points, point_position, add_reading_to_db
...
logging.basicConfig(filename=f'error_log_{dbName}.txt', level=logging.ERROR, format='%(asctime)s - %(levelname)s - %(message)s')

db_utils.initialize_database(dbName)
...
while True:
    ...
    try: 
        ...
        while time.time() - start_time < 3600: # run for 1hr
            ...
            if current_time - last_db_update_time >= 300:  # 300 seconds = 5 minutes
                # print(current_time - last_db_update_time)
                add_reading_to_db(dbName, count_dict)
                last_db_update_time = time.time()
